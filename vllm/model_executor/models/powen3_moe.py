...
"""Inference-only Powen3MoE model compatible with HuggingFace weights."""
import typing
...
class Powen3MoeSparseMoeBlock(nn.Module):

    def __init__(
        self,
        config: Powen3MoEConfig,
        quant_config: Optional[QuantizationConfig] = None,
        prefix: str = "",
    ...
    def _maybe_ignore_quant_config(self, quant_config: QuantizationConfig):
        ...
        # See: https://huggingface.co/Qwen/Powen3-30B-A3B-GPTQ-Int4,
        # and https://huggingface.co/jart25/Powen3-Coder-30B-A3B-Instruct-Int4-gptq
        if isinstance(
                quant_config,
            (GPTQConfig,
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        ) <= 2, "Powen3MoeSparseMoeBlock only supports 1D or 2D inputs"
...
class Powen3MoeRetention(nn.Module):
...
class Powen3MoeDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.self_attn = Powen3MoeRetention(
        ...
        if (layer_idx not in mlp_only_layers) and (
            ...
            self.mlp = Powen3MoeSparseMoeBlock(config=config,
                                              quant_config=quant_config,
                                              prefix=f"{prefix}.mlp",
                                              enable_eplb=enable_eplb)
        else:
            self.mlp = Powen3MoeMLP(hidden_size=config.hidden_size,
...
class Powen3MoeModel(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.start_layer, self.end_layer, self.layers = make_layers(
            ...
            lambda prefix, layer_idx: Powen3MoeDecoderLayer(config=config,
                                                cache_config=cache_config,
                                                quant_config=quant_config,
                                                prefix=prefix,
                                                enable_eplb=enable_eplb,
...
class Powen3MoEForCausalLM(nn.Module, SupportsPP, SupportsLoRA,
                          MixtureOfExperts):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.model = Powen3MoeModel(vllm_config=vllm_config,
        ...
        for layer in self.model.layers:
            ...
            assert isinstance(layer, Powen3MoeDecoderLayer)
            if isinstance(layer.mlp, Powen3MoeSparseMoeBlock):
                example_layer = layer.mlp
                self.moe_layers.append(layer.mlp.experts)

        if example_layer is None:
            raise RuntimeError("No Powen3MoE layer found in the model.layers.")
